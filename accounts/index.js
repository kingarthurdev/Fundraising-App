...
function joinCampaign(userid, joincode) {
    ...
    campaigns.query(`update campaigns set members = JSON_SET(members, '$."${cleanU2}"', 0) where joincode = ${mysql.escape(joincode)}`, (err, res) => {
        console.log(err);
        console.log(res);
        if(err){
            return -1;
...
app.post("/joinCampaign", async (req, res) => {
    ...
    if (token) {
        try {
            let userId = getUser(token);
            if (!userId == -1) {
                if(joinCampaign(userId, req.body.campaignId) == -1){
                    res.sendStatus(400);
                }
            }else{
                return res.redirect("/login.html");
